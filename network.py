...
exec(open('network_utilities_ver4.py').read())
...
ib = pd.read_csv('signal_pathway.csv')

#  Construct Network (STRING network v11)
print('constructing STRING PPI network, %s'% time.ctime())
tmp_G = nx.Graph()
annotation = pd.read_csv('9606.protein.aliases.v12.0.txt', sep='\t') # STRING network ensembl-geneID mapping
net = pd.read_csv('9606.protein.links.v12.0.txt', sep=' ') # STRING network
nodes1 = net.values[:,0]
nodes2 = net.values[:,1]
scores = net.values[:,2]
...
for biomarker, feature in zip(ib['Name'].tolist(), ib['Feature'].tolist()):
        if '%s.txt'%biomarker in os.listdir():
                continue
        ...
        output.to_csv('%s.txt' % biomarker, sep='\t', index=False)
